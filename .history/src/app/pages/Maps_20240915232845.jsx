...
const Maps = () => {
  ...
  const [coords, setCoords] = useState({ lat: 0, lng: 0 }); // Initialize with 0
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          ({ coords: { latitude, longitude } }) => {
            setCoords({ lat: latitude, lng: longitude }); // Set user's current location
          },
          (error) => {
            console.error("Error getting user location:", error);
            // Fallback to a default location (Melbourne)
        ...
        setCoords({ lat: -37.8136, lng: 144.9631 }); // Default to Melbourne
      }
    };
    getUserLocation();
  }, []);
    ...
    <div className="flex flex-col lg:flex-row h-full min-h-screen pt-[80px] pb-8">
      ...
      <div className="lg:w-4/5 w-full lg:h-full h-1/2">
        <Map
          ...
          setCoords={setCoords}
          setBounds={setBounds}
          coords={coords} // Use user's current location or default
