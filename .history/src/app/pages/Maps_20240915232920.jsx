...
const Maps = () => {
  ...
  // Function to fetch IP-based location if geolocation fails
  const getIPLocation = async () => {
    try {
      ...
      // Default to some place in case IP lookup also fails
      setCoords({ lat: 0, lng: 0 });
    }
  };

  // Set initial coordinates based on user's geolocation or IP-based location
  useEffect(() => {
    if (navigator.geolocation) {
      ...
      // If geolocation is not supported, fallback to IP-based location
      getIPLocation();
    }
  }, []);
    ...
    <div className="flex flex-col lg:flex-row h-full min-h-screen pt-[80px] pb-8">
      ...
      <div className="lg:w-4/5 w-full lg:h-full h-1/2">
        <Map
          ...
          setCoords={setCoords}
          setBounds={setBounds}
          coords={coords} // Use user's current location or fallback location
