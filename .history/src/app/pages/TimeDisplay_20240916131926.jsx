...
import LocalTimeRing from "./LocalTimeRing";
...
const TimeDisplay = () => {
  ...
  return (
    <div className="flex flex-col items-center mt-12">
      <h1 className="text-3xl font-bold mb-6">Current Time & International Times</h1>
      ...
      <div className="grid grid-cols-3 grid-rows-2 gap-4 items-center">
        ...
        {cityTimes.map((city, index) => (
          <TimeRing key={index} city={city.name} time={city.time} ringColor={getRingColor(index)} />
        ))}
      </div>
    </div>
  );
...
const getRingColor = (index) => {
  const colors = ["border-blue-500", "border-black", "border-red-500", "border-yellow-500", "border-green-500"];
