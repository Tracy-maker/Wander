...
const Weather = () => {
  ...
  const [highlight, setHighlight] = useState(false); // State for highlighting
    ...
    Promise.all([getCurrentWeather(lat, lon), getDailyForecast(lat, lon)])
      .then(([currentWeatherResponse, forecastResponse]) => {
        if (currentWeatherResponse && forecastResponse) {
          ...
          setHighlight(true); // Trigger highlight after successful fetch
        }
      })
  ...
  return (
    ...
    >
      ...
      <div className="fixed top-10 left-1/2 transform -translate-x-1/2 z-50 w-full max-w-4xl flex justify-center items-center space-x-4">
  <div className="flex-1">
    <input
      type="text"
      placeholder="Enter location..."
      className={`w-full px-4 py-2 text-lg border rounded-lg focus:outline-none ${
        highlight ? "ring-2 ring-green-400" : "focus:ring-2 focus:ring-blue-400"
      }`}
