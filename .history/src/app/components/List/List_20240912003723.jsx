...
const List = ({ places, childClicked }) => {
  ...
  const [placeRefs, setPlaceRefs] = useState([]); // 'placeRefs' stores the list of refs for each place

  useEffect(() => {
    // Create or reuse refs for each place in the 'places' array
    const newRefs = places.map((_, index) => {
      // Reuse an existing ref if it exists, or create a new one if it doesn't
      return placeRefs[index] || createRef();
    });

    // Store the new list of refs in the state
    setPlaceRefs(newRefs);
  }, [places]); // This effect runs whenever the 'places' array changes
