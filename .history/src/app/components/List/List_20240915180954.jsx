import React, { createRef, useEffect, useState } from "react";
import PlaceDetails from "../PlaceDetails/PlaceDetails";
import Filters from "../Filters/Filters";
import { FaSpinner } from "react-icons/fa";

const List = ({
  places,
  childClicked,
  isLoading,
  type,
  rating,
  setRating,
  setType,
}) => {
  const [placeRefs, setPlaceRefs] = useState([]);

  // Create references for each place in the list
  useEffect(() => {
    const newRefs = Array(places?.length)
      .fill()
      .map((_, index) => placeRefs[index] || createRef());

    setPlaceRefs(newRefs);
  }, [places]);

  return (
    <div className="flex flex-col h-full w-full rounded-lg shadow-md max-w-screen-lg mx-auto">
      {/* Loading Spinner */}
      {isLoading ? (
        <div className="flex justify-center items-center h-[600px]">
          <FaSpinner className="animate-spin text-4xl text-blue-500" />
        </div>
      ) : (
        <div className="flex-grow mt-4 p-2">
          {/* Filters */}
          <Filters type={type} rating={rating} setRating={setRating} setType={setType} />

          {/* Scrollable list of places */}
          <div className="flex flex-col gap-4 mt-2 overflow-y-auto h-[605px] scrollbar-thin scrollbar-thumb-blue-500 scrollbar-track-gray-200">
            {places?.map((place, i) => (
              <div key={i} className="bg-white rounded-lg p-2">
                <PlaceDetails
                  place={place}
                  selected={Number(childClicked) === i}
                  refProp={placeRefs[i]}
                />
              </div>
            ))}
          </div>
        </div>
      )}
    </div>
  );
};

export default List;
