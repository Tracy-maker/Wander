...
const List = ({ places, childClicked }) => {
  ...
  useEffect(() => {
    // Create an array of refs, one for each place in the 'places' array
    const refs = places.map((_, i) => {
      // Reuse the existing ref if it exists, otherwise create a new one
      return elRefs[i] || createRef();
    });
  
    // Save the new refs to the state
    setElRefs(refs);
  }, [places]); // Run this effect whenever 'places' changes
