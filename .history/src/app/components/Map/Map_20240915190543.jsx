...
const Map = ({ setCoords, setChildClicked, setBounds, coords, places }) => {
  const isDesktop = window.matchMedia("(min-width: 600px)").matches; // For rendering the correct marker style

  return (
    <div className="h-[87vh] w-full">
      <GoogleMapReact
        bootstrapURLKeys={{ key: APP_GOOGLE_MAPS_API_KEY }}
        center={coords} // Dynamically set map center based on updated coords
        defaultZoom={14}
        margin={[50, 50, 50, 50]}
        ...
        onChildClick={(child) => {
          setChildClicked(child); // Handle child marker click event
        }}
      >
        {places.length &&
          places.map((place, i) => (
