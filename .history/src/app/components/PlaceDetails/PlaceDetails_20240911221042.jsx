    ...
    <div className="bg-white shadow-lg rounded-lg overflow-hidden mb-6">
      ...
      <div className="p-4">
        ...
        {place?.awards?.map((award) => (
          <div key={award.display_name} className="flex justify-between items-center my-2">
        ...
        <div className="flex flex-wrap mt-3">
          {place?.cuisine?.map(({ name }) => (
            <span key={name} className="bg-gray-200 text-sm rounded-full px-3 py-1 mr-2 mb-2">
              {name}
            </span>
          ))}
        </div>
      ...
      <div className="p-4 flex justify-between">
        <button
          className="bg-blue-500 text-white px-3 py-1 rounded hover:bg
